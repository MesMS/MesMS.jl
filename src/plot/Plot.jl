...
calc_ion(base, δ, z, type, color=:green; type_abbr=type) = begin
    (;
        base..., type, mz=(base.mass + δ + z * MesMS.mₚ) / z, z, color,
        text="\$$(type)_{$(base.idx)}^{$(z == 1 ? "" : z)+}\$",
        text_abbr="\$$(type_abbr)_{$(base.idx)}^{$(z == 1 ? "" : z)+}\$",
    )
end
...
build_ions(spec, seq, mods, ε, tab_ele, tab_aa, tab_mod; types=[(ion_b, 1:2), (ion_y, 1:2)], abu=false) = begin
    ...
    ions = [calc_ion(i, MesMS.calc_mass(t.Δ, tab_ele), z, t.sym, t.color) for (t, zs) in types for z in zs for i in base[t.part]]
...
build_ions_xl(spec, seqs, modss, linker, sites, ε, tab_ele, tab_aa, tab_mod; types=[(ion_b, 1:3), (ion_y, 1:3)], abu=false) = begin
    ...
    ions = map(zip((:α, :β), lsums, sites, anothers, (0.25, -0.25), (:circle, :diamond))) do (n, s, site, another, offset, shape)
        if linker.cleavable
            ...
            s[site:end] .+= another + linker.mass + MesMS.calc_mass(MesMS.Formula(H=2, O=1), tab_ele)
            base = calc_base(s)
        end
        ions = [calc_ion(i, MesMS.calc_mass(t.Δ, tab_ele), z, "$(n)$(i.var)$(t.sym)", t.color; type_abbr="$(i.var)$(t.sym)") for (t, zs) in types for z in zs for i in base[t.part]]
        ions = [match_peak(spec, ion, ε; abu) for ion in ions]
